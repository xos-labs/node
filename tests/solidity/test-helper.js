...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print xosd output, default false').argv
...
function setupNetwork ({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    ...
    const serverStartedMsg = 'xosd started'
    ...
    logger.info(`Starting xosd process... timeout: ${timeout}ms`)
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start xosd timeout!')), timeout)
  })
