...
import (
	...
	"github.com/xos-labs/node/cmd/xosd/config"
...
func setup(withGenesis bool, invCheckPeriod uint, chainID string) (*XOSD, GenesisState) {
	db := dbm.NewMemDB()
...
// Setup initializes a new XOSD. A Nop logger is set in XOSD.
func Setup(t *testing.T, chainID string) *XOSD {
	t.Helper()
...
// SetupWithGenesisValSet initializes a new XOSD with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit in the default token of the simapp from first genesis
// account. A Nop logger is set in XOSD.
