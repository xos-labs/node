...
func (s *PrecompileUnitTestSuite) SetupTest(chainID string) {
	...
	s.precompileAddrHex = network.GetWTOKENContractHex(chainID)
	...
	s.Require().True(found, "expected wtoken precompile to be registered in the tokens map")
...
func (s *PrecompileUnitTestSuite) TestEmitDepositEvent() {
	testCases := []struct {
		...
		{
			name:    "mainnet",
			chainID: testconstants.AppChainID,
		}, {
			name:    "six decimals",
