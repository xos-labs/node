...
import (
	...
	auth "github.com/xos-labs/node/precompiles/authorization"
	"github.com/xos-labs/node/precompiles/erc20"
	"github.com/xos-labs/node/precompiles/testutil"
	"github.com/xos-labs/node/precompiles/werc20"
	"github.com/xos-labs/node/precompiles/werc20/testdata"
...
func TestPrecompileIntegrationTestSuite(t *testing.T) {
	...
	RunSpecs(t, "WTOKEN precompile test suite")
}
...
var _ = When("a user interact with the WTOKEN precompiled contract", func() {
	var (
	...
	BeforeEach(func() {
		...
		is.precompileAddrHex = network.GetWTOKENContractHex(is.network.GetChainID())

		ctx := integrationNetwork.GetContext()

		// Perform some check before adding the precompile to the suite.

		// Check that WTOKEN is part of the native precompiles.
		...
		Expect(erc20Params.NativePrecompiles).To(
			...
			"expected wtoken to be in the native precompiles",
		)
		...
		// Check that WTOKEN is registered in the token pairs map.
		...
		Expect(found).To(BeTrue(), "expected wtoken precompile to be registered in the tokens map")
		...
		revertCallerContract, err := testdata.LoadWTOKEN9TestCaller()
	...
	Context("calling an erc20 method", func() {
		...
		When("querying information", func() {
			...
			It("should return the decimals", func() {
				...
				coinInfo := testconstants.AppChainCoinInfo[is.network.GetChainID()]
