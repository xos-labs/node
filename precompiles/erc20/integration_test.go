...
import (
	...
	"github.com/xos-labs/node/precompiles/erc20/testdata"
	...
	integrationutils "github.com/xos-labs/node/testutil/integration/os/utils"
...
var _ = Describe("ERC20 Extension -", func() {
	...
	BeforeEach(func() {
		...
		Expect(common.HexToAddress(erc20Params.NativePrecompiles[0])).To(Equal(common.HexToAddress(testconstants.WTOKENContractMainnet)))
	...
	Context("basic functionality -", func() {
		...
		When("calling reverter contract", func() {
			Context("in a direct call to the WTOKEN contract", func() {
				var (
				...
				DescribeTable("it should revert token transfer from the WTOKEN contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					amountToSend := big.NewInt(100)
				...
				It("it should send token transfer and send from WTOKEN contract", func() {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
				...
				DescribeTable("it should revert token transfer and send from WTOKEN contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					balRes, err := is.handler.GetBalanceFromBank(receiver.Bytes(), is.bondDenom)
					Expect(err).To(BeNil())
...
var _ = Describe("ERC20 Extension migration Flows -", func() {
	...
	When("using Evmos (not wToken) in smart contracts", func() {
