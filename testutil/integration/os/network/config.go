...
import (
	...
	testtx "github.com/xos-labs/node/testutil/tx"
...
var defaultChain = testconstants.AppChainID
...
func WithChainID(chainID string) ConfigOption {
	...
	evmCoinInfo, found := testconstants.AppChainCoinInfo[chainID]
	if !found {
		panic(fmt.Sprintf(
			"chain id %q not found in chain coin info; available: %v",
			chainID,
			testconstants.AppChainCoinInfo,
		))
	}
